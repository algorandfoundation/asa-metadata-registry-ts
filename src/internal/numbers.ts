 ...
 */
export const toUint64BigInt = (v: unknown, name: string): bigint => {
  if (typeof v === 'bigint') {
    if (v < 0n || v > MAX_UINT64) throw new RangeError(`${name} must fit in uint64`)
    return v
  }
  if (typeof v === 'number') {
    const bi = toNonNegativeBigInt(v)
    if (bi > MAX_UINT64) throw new RangeError(`${name} must fit in uint64`)
    return bi
  }
  if (typeof v === 'string' && /^[0-9]+$/.test(v)) {
    const x = BigInt(v)
    if (x > MAX_UINT64) throw new RangeError(`${name} must fit in uint64`)
    return x
  }
  throw new TypeError(`${name} must be uint64`)
}
...
export const asNumber = (v: unknown, name: string): number => {
  if (typeof v === 'number') {
    ...
    if (!Number.isSafeInteger(v)) throw new RangeError(`${name} is too large for JS number`)
    return v
  }
  if (typeof v === 'bigint') {
    if (v < BigInt(Number.MIN_SAFE_INTEGER) || v > BigInt(Number.MAX_SAFE_INTEGER)) {
      throw new RangeError(`${name} is too large for JS number`)
    }
    return Number(v)
  }
...
export const asUint8 = (v: unknown, name: string): number => {
  ...
  if (n < 0 || n > MAX_UINT8) throw new RangeError(`${name} must fit in uint8`)
  return n
}